...
class BaseNode(base.MemoryModule):
    def __init__(self, v_threshold: float = 1., v_reset: float = 0.,
        ...
        * :ref:`中文API <BaseNode.__init__-cn>`
        ...
        # self.dropout = CustomDropout(p=0.02)
...
class MultiStepLIFNode(LIFNode):
    def __init__(self, timestep, tau: float = 2., decay_input: bool = True, v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        if self.backend == 'torch':
            ...
            if isinstance(self.v, float):
                v_init = self.v
                self.v = torch.zeros_like(x_seq[0].data)
                # self.v = torch.rand_like(x_seq[0].data) * 0.2
                if v_init != 0.:
                    torch.fill_(self.v, v_init)
            ...
            self.v = self.v_seq[-1].clone()
            #spike_seq = self.dropout(spike_seq)
